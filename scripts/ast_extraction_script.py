...
class MethodInfo:
    def __init__(
        self, name, signature, code_type, docstring, line, line_from, line_to, context
    ):
        self.name = name
...
@click.group()
def extract():
    """Knowledge base related commands."""
    pass
...
def run_extraction(folder_path, output_path):
    all_methods = process_folder(folder_path)

    with open(output_path, "w") as json_file:
        json.dump(all_methods, json_file, indent=4)

    print(f"Extracted methods from {folder_path} have been saved to {output_path}")
